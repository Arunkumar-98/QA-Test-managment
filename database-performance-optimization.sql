...
-- Function to search test cases with full-text search
...
BEGIN
  ...
  SELECT 
    ...
    tc.updated_at,
    ts_rank(
      to_tsvector('english', tc.test_case || ' ' || tc.description || ' ' || tc.steps_to_reproduce),
  ...
  WHERE tc.project_id = project_uuid
    AND (
      ...
      to_tsvector('english', tc.test_case || ' ' || tc.description || ' ' || tc.steps_to_reproduce) @@ plainto_tsquery('english', search_query)
    )
...
-- Create a view to monitor slow queries (if pg_stat_statements is enabled)
-- Note: This view may not work in all Supabase instances due to pg_stat_statements availability
