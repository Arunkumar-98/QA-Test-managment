...
import { Loader2, Share2, Copy, Eye, MessageSquare, Edit, Plus, Trash2, Download, Calendar, Users, EyeOff, Folder } from 'lucide-react'
import { useToast } from '@/hooks/use-toast'
import { TestSuitePermissions, TestSuiteShare, TestSuite } from '@/types/qa-types'
import { testSuiteShareService } from '@/lib/supabase-service'

interface ShareTestSuiteDialogProps {
  isOpen: boolean
  onClose: () => void
  testSuite: TestSuite
  projectId: string
  projectName: string
  onShareCreated?: (share: TestSuiteShare) => void
}

const PERMISSION_PRESETS = {
  'view-only': {
    name: 'View Only',
    description: 'Can only view test cases in this suite',
    permissions: {
      canView: true,
      canComment: false,
      canEdit: false,
...
export function ShareTestSuiteDialog({ 
  isOpen, 
  onClose, 
  testSuite, 
  projectId, 
  projectName, 
  onShareCreated 
}: ShareTestSuiteDialogProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [selectedPreset, setSelectedPreset] = useState<string>('view-only')
  const [permissions, setPermissions] = useState<TestSuitePermissions>(PERMISSION_PRESETS['view-only'].permissions)
  const [expiresAt, setExpiresAt] = useState<string>('')
  const [maxViews, setMaxViews] = useState<string>('')
  const [allowedEmails, setAllowedEmails] = useState<string>('')
  const [createdShare, setCreatedShare] = useState<TestSuiteShare | null>(null)
  const [error, setError] = useState('')
  
  const { toast } = useToast()

  // Safety check for null testSuite
  if (!testSuite) {
    return null
  }
  ...
  const handlePermissionChange = (permission: keyof TestSuitePermissions, value: boolean) => {
  ...
  const handleCreateShare = async () => {
    ...
    try {
      ...
      const share = await testSuiteShareService.createShare(
        testSuite.id, 
        testSuite.name, 
        projectId, 
        projectName, 
      ...
      toast({
        title: "Test Suite Shared Successfully!",
        description: "Your test suite has been shared. Copy the link below to share with others.",
      })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to create share link')
      toast({
        title: "Error Sharing Test Suite",
  ...
  const copyShareLink = async () => {
    ...
    const shareUrl = `${window.location.origin}/shared-suite/${createdShare.accessToken}`
      ...
      <DialogContent className="max-w-4xl w-[90vw] max-h-[95vh] overflow-y-auto bg-white border border-slate-200 shadow-2xl">
        ...
        {!createdShare ? (
            ...
            <Card className="bg-slate-50">
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-medium">{testSuite.name}</h3>
                    <p className="text-sm text-slate-600">{testSuite.description || 'No description'}</p>
                    <div className="flex items-center space-x-4 mt-2 text-xs text-slate-500">
                      <span>📊 {testSuite.totalTests || 0} test cases</span>
                      <span>✅ {testSuite.passedTests || 0} passed</span>
                      <span>❌ {testSuite.failedTests || 0} failed</span>
                      <span>⏳ {testSuite.pendingTests || 0} pending</span>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="space-y-4">
            <Alert>
              <AlertDescription>
                Your test suite has been shared successfully! Share the link below with your team members.
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                <Input
                  value={`${window.location.origin}/shared-suite/${createdShare.accessToken}`}
                  readOnly
                  className="flex-1"
                />
